...
- This mirrors the `yoke:explorer` agent's fallback rules so ad-hoc sessions and the `/yoke:work`
...
- **Two apps, two staging targets.** `staging.idcredentor.org` serves **`apps/web` only**; the admin app has its own host, `staging.ministerio.idcredentor.org` (a separate Vercel project). Post-merge QA picks the right one(s) from the merged diff via `merge.postMergeQa.byPath` (`apps/admin` → `stagingAdmin`, `apps/web` → `staging`, `packages` → **both** — the shared workspace packages are compiled into both apps, so QA runs against every matched env). Aiming an `apps/admin` run at the website's staging host exercises none of the change and still reports PASS — see `docs/architecture/agent-harness.md` § QA environments.
- **No Storybook.**
...
2. **`tasks/specs/`** — local per-ticket artifacts written during a `/yoke:work` run.

### Harness commands (yoke plugin)

- `/yoke:pm` — intake / refine / groom a Jira issue against `docs/product/` (human-gated; never past To Do).
- `/yoke:work ICR-N` — the orchestrator: mandatory worktree + branch → explore → (conditional design gate) → plan → implement (TDD) ↔ verify → draft PR → preview QA → mark ready → detached post-PR review + CI loop. Owns exactly **two** Jira transitions: To Do→In Progress and In Progress→In Review.
- `/yoke:qa ICR-N` — acceptance QA (staging by default; `--preview` targets the PR's Vercel preview). Report-only; posts a structured Jira comment.
- `/yoke:verify` — `pnpm type-check` + `pnpm lint` + `pnpm test` (+ `pnpm build`) plus security checks.
- `/yoke:merge ICR-N` — **human-triggered** squash-merge → In Review→In Testing → post-merge **staging** QA. Never deploys prod, never sets Done.

### Artifacts per ticket

- **Spec:** `tasks/specs/ICR-N-<slug>.md` — requirements and design.
- **Plan:** `tasks/specs/ICR-N-<slug>.plan.md` — file paths, checkpoints, dependencies (plus the per-run `.state.json` for `/yoke:work` resume).

### The two human gates

1. A conditional **design gate** inside `/yoke:work` (brainstorm + spec for non-trivial or sensitive work — the six sensitive areas `email-services`, `form-pii-spam`, `likes-mongo`, `env-secrets`, `csp-headers`, `i18n-messages` always gate).
2. The **merge** trigger — you say "merge"; `/yoke:merge` runs. Afterward **you** deploy prod and move the issue to **Done** (Done is human-only; no agent ever sets it).

### Contentful model-change gate (domain)

`ICR` is Contentful-backed, so `/yoke:work` carries one extra project gate. **When a `/yoke:work` plan changes the Contentful content model** — creates / updates / deletes a **content type or field**, or **remaps entries** (as opposed to only adding a read-side GraphQL fragment/getter in `lib/contentful/*`) — STOP after the plan and confirm the migration **lane** before implementing:
...
- **Engineering docs** — in `docs/architecture/`:
  ...
  - `code-patterns-and-conventions.md` — the observed "how it's actually written" code-review (2026-06-22, **validated 2026-07-20**): the RSC/fetch-in-page mental model, the shadcn↔hand-rolled UI seam, per-finding status + the Jira map (Epic ICR-9 / ICR-30→38). The §10 "candidates to standardize" list is the payload for ICR-38.
  ...
  - `design-system.md` — the shared design system for both apps: the token contract, derived tokens, the primitives-shared/features-local boundary, the per-primitive shadcn→`@idcr/ui` migration table, the `@source` scanner prerequisite, and the known token/Divider defects.
  ...
  - `admin-auth.md` — `apps/admin`'s native Firebase session-cookie auth: the sign-in → `POST`/`DELETE /api/auth/session` → cookie lifecycle, `getCurrentUser` (authoritative, `checkRevoked:true`) vs the proxy (fast convenience, `checkRevoked:false`), the invite-only provisioning gate (and why the client never deletes a Firebase credential on a refused sign-in), roles/`preferredLocale` resolved from Mongo (never the token), the per-user language preference lifecycle, the deliberate divergences from `divinelab/toulmin-lab`'s NextAuth/custom-claims model, and the preview-vs-staging-vs-manual QA verifiability boundary.
  - `admin-rbac.md` — `apps/admin`'s granular RBAC (ICR-128): the 15-key `PERMISSIONS` const-map registry and its reject-at-write/ignore-at-read symmetry, `requirePermission()` as the one IO boundary (and why permissions are never cached in the token or session cookie, for instant mid-session revocation), the last-admin invariant as a post-state predicate over five mutation paths (not an admin count), the `withAdminTransaction` rules and the `abortTransaction`-as-refusal pattern, the audit log and why `users:manage`/`roles:manage` are admin-equivalent by construction, the `people:pii` field-omission helper's scope boundary (enforcement primitive here, real DTOs in ICR-129), and the surfaces deliberately left ungated.
  ...
  - `admin-navigation.md` — `apps/admin`'s responsive nav (ICR-126): the single `NAV_ITEMS` module and its adaptive 5-flat/4+More `splitNavItems()` rule, why `NavLink` takes pre-rendered `children` and never a `NavItem` (the RSC function-prop boundary), why `usePathname` must come from `@src/i18n/routing` and not `next/navigation`, permission filtering resolved once in `AppShell` as convenience only, the `fixed` tab bar + `main`-padding layout choice, and the PWA manifest's `proxy.ts` `webmanifest` bypass coupling.
