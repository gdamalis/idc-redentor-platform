...
7. **Named `interpreter` present with `interpreted` absent/false** → `validateSermonForEntry()` **refuses** (fail-closed, added in the ICR-149 review pass; mirrors the voice guard's `check-voice-learn.mjs` rule that a named interpreter is itself a declaration). A stray interpreter object is treated as a forgotten `"interpreted": true` flag rather than silently dropped — the sermon.json must be fixed and re-dispatched instead of publishing without its badge/credit.
8. **`interpreter.email` present but blank/whitespace-only** → the validator **rejects** it (must be a non-empty string when present), so the publisher's `interpreter.email ?? "info@idcredentor.org"` fallback never sees `""` (`??` does not fall through on empty string — the ICR-117 lesson). A present-but-non-blank, syntactically malformed email is still deferred to the Contentful `author.email` regex, the final gate on PUBLISH (human Gate 2) — we do not re-implement that regex in the validator (single source of truth = Contentful).
