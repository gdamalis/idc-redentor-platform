...
- [ ] **Step 2 — write `docs/architecture/admin-auth.md`.** Cover: the native session-cookie flow (POST/DELETE, auth_time recency, cookie attrs, 5-day expiry); `getCurrentUser` (checkRevoked:true) vs proxy (checkRevoked:false) and why the RSC layout is the authoritative gate; the invite gate; roles-from-Mongo-never-token; the per-user `preferredLocale` (invite-seeded, login-applied, switcher-persisted); the deliberate divergences from `toulmin-lab` (native cookie not NextAuth, roles from Mongo not custom claims, return values not thrown Errors); the QA verifiability boundary (preview = UI/proxy/API-rejection; staging = live email/pw happy-path; Google OAuth = manual). Follow the `divinelab:scribe` doc standard. **[Post-merge amendment, PR #109]:** this checkpoint originally also documented an orphan-Firebase-credential cleanup on `no-invite`; that cleanup was removed after an expert review, and the doc now covers why instead (see `tasks/specs/ICR-127-admin-firebase-auth.md` § 11 item 6).
