...
| `apps/admin/src/app/[locale]/no-access/page.tsx`             | localized no-access + back-to-login / sign-out                                              |
...
| `apps/admin/messages/{es-AR,en-US}.json`                     | add `auth.*` namespace (both, parity)                                                       |
| `apps/admin/src/i18n/messages.test.ts`                       | assert `auth.*` parity                                                                      |
| `apps/admin/.env.example`                                    | comment block: which vars the auth flows consume (no new vars)                              |
...
[locale]/no-access/page.tsx (RSC, localized) → back-to-login / sign-out
...
- `auth.noAccess`: `title`, `description`, `backToLogin`, `signOut`.
