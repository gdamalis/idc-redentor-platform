...
| #   | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1  | **Design-only. No production code.** The `@idcr/ui` code migration is a separate follow-up ticket. **Amended 2026-07-27 (maintainer decision, PR #112 review):** the destructive-contrast WCAG fixes are brought into scope — light `--destructive` darkened and a new `--destructive-text` token in `packages/ui/src/tokens.css`, the three small-foreground-text rewires in the apps, and the `@idcr/ui` patch changeset. All other production code remains out of scope; the primitives-as-code migration remains ICR-37. |
...
| R9  | **No production code changes.** `apps/web/**`, `apps/admin/**` and `packages/ui/**` are untouched. The one permitted exception is adding the new doc to the `CLAUDE.md` + `AGENTS.md` doc indexes. **Amended 2026-07-27:** one further exception, per the maintainer's scope decision recorded in D1 — the destructive-contrast fix (`tokens.css` destructive values, `--destructive-text`, three foreground-text rewires, changeset). |
