...
For each mapped project run `pnpm e2e --project=<name>` via `Bash` with a 600s timeout (at `light`, the mapped project(s) for the primary AC route(s); `standard`/`heavy` widen the set). Capture the exit code + the last ~80 lines of output per failing test (Playwright's `list` reporter output is fine).
