...
- `featuredImageNote` — none is generated; the human attaches one at Gate 2.
...
8. **Defer featuredImage only.** The pipeline generates no featuredImage; report that the human attaches one
   at Gate 2 (it is required before publishing). audio + both PDFs are already attached.
