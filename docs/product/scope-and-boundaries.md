...
- **Newsletter signup** — email capture to a per-locale **Resend** audience (`/api/subscribe` → `subscribe.service.ts` → `resendAudience.ts`). The subscriber's email is stored at Resend, **never in our own database**.
