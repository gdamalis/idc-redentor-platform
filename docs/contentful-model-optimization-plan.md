...
>
> This plan ships as **one atomic epic PR + one Contentful cutover**. All model + entry changes are made in `agent-sandbox` via committed migration scripts, the whole site is tested locally (and on a sandbox-pointed preview), and at the end the `master` alias is re-pointed to `agent-sandbox` at the same moment the PR merges.
...
| # | Decision | Choice |
|---|---|---|
| D1 | **Cutover mechanism** | **Re-point the `master` alias → `agent-sandbox`.** `master-0.0.1` stays as instant rollback. Model + migrated entries go live atomically. |
| D2 | **How changes are made & recorded** | **Committed migration scripts** (`contentful-migration`) under `scripts/contentful/`, run against `agent-sandbox` with the existing CMA token. Reviewable in the PR, idempotent, replayable. |
...
- `master` is an **alias → `master-0.0.1`** (production, created 2024-05-31). `agent-sandbox` is a **fresh, byte-for-byte clone** created 2026-06-22 (same schema + entry IDs/versions; not stale).
...
- **Contentful MCP** scoped to `agent-sandbox` with `PROTECTED_ENVIRONMENTS=master` (available; used for inspection + verification).
- **`agent-sandbox` is a current clone of `master-0.0.1`.** Confirmed fresh (2026-06-22). If production content drifts during the work window, re-clone before cutover (migration scripts are idempotent → re-run cheaply).
- **Vercel Preview env var:** the epic branch's preview must set `CONTENTFUL_ENVIRONMENT=agent-sandbox` (branch-scoped) so the preview renders the new model. See §3.
...
- Any environment running epic code **must** point at `agent-sandbox`, or it renders the old model against new code and breaks.
...
**To test locally against the sandbox:** set `CONTENTFUL_ENVIRONMENT=agent-sandbox` in `.env.local`.
...
Pre-flight: code + sandbox fully verified (full local pass against `agent-sandbox`, plus the sandbox-pointed Vercel preview), `pnpm type-check && pnpm lint && pnpm test && pnpm build` green, content edits frozen (or re-sync done).

**Recommended — planned low-traffic window (simple; ~1–2 min blip acceptable for this site):**
1. Pick a low-traffic minute.
2. Re-point the `master` alias → `agent-sandbox` in Contentful (Settings → Environments → Environment Aliases).
...
**Post-cutover housekeeping:** the alias now points at `agent-sandbox`, so that env **is** production. Clone a **new** `agent-sandbox` from the new master and update the MCP's `ENVIRONMENT_ID` for future work. (Cosmetic: production env id is now literally `agent-sandbox`; acceptable, or rotate to a clean name on the next cycle.)
...
- **Target env:** runner reads `CONTENTFUL_ENVIRONMENT` (default `agent-sandbox`) + `CONTENTFUL_MANAGEMENT_ACCESS_TOKEN` + `CONTENTFUL_SPACE_ID`. **Never targets `master`.**
...
- **Verify:** `CONTENTFUL_ENVIRONMENT` unset → identical prod behavior; set to `agent-sandbox` → site renders from sandbox locally; runner connects to sandbox.
...
- **Per commit:** `pnpm type-check && pnpm lint && pnpm test && pnpm build` green; targeted manual smoke of the affected route(s) **locally against `agent-sandbox`**.
...
- **Full pre-cutover pass:** run the whole site against `agent-sandbox` locally **and** on the sandbox-pointed Vercel preview; walk home, who-is-jesus, community (Creed + PhotoGrid), come-meet-us (event/map/contact form/verse), blog index + a post, newsletter signup, the new privacy page. Confirm JSON-LD via a validator.
...
2. **Content drift during the work window.** If editors change production content, re-clone `agent-sandbox` and re-run the idempotent scripts before cutover.
...
7. **Post-cutover sandbox rotation** — clone a new `agent-sandbox` and update the MCP `ENVIRONMENT_ID`.
