...
**Architecture:** All Contentful model + entry changes are made in the `master-1.0.0` environment via committed, idempotent `contentful-migration` scripts (`scripts/contentful/`). The app gains a `CONTENTFUL_ENVIRONMENT` override so local + the epic's Vercel preview read `master-1.0.0` while production keeps reading the `master` alias. Code and model move together; at the end a human re-points the `master` alias → `master-1.0.0` at the same moment the PR merges. `master-0.0.1` stays as instant rollback.
...
- **Migrations never target `master`.** The runner uses `CONTENTFUL_ENVIRONMENT` (default `master-1.0.0`); the MCP enforces `PROTECTED_ENVIRONMENTS=master`.
...
1. **Confirm live shape** — use the Contentful MCP (`get_content_type`, `search_entries`) against `master-1.0.0` to confirm the current fields/entries the migration will touch. Never assume; the audit is a guide, the live model is truth.
...
3. **Dry-run then apply** — `node scripts/contentful/run.mjs <NN> --dry-run` then without the flag, against `master-1.0.0`.
4. **Verify in-CMS** — re-query via MCP to confirm the model/entries match the target.
5. **Verify rendered** — run the app locally against `master-1.0.0` (`CONTENTFUL_ENVIRONMENT=master-1.0.0 pnpm dev`), smoke the affected route(s).
...
- Produces: `fetchGraphQL(query, preview)` now targets `process.env.CONTENTFUL_ENVIRONMENT ?? "master"`. The runner `node scripts/contentful/run.mjs <NN> [--dry-run]` applies `scripts/contentful/migrations/<NN>-*.cjs` to `$CONTENTFUL_ENVIRONMENT` (default `master-1.0.0`).
...
```bash
# Optional. Defaults to the `master` alias. Set to `master-1.0.0` locally to test model changes.
...
- [ ] **Step 8: Verify the runner connects (no-op).** Create a throwaway `scripts/contentful/migrations/00-noop.cjs` exporting `module.exports = function (migration) {};`, run `CONTENTFUL_ENVIRONMENT=master-1.0.0 node scripts/contentful/run.mjs 00 --dry-run`, confirm it connects and reports no changes, then delete `00-noop.cjs`.
...
module.exports = function (migration, { makeRequest }) {
  ...
  cta.editField("subline").validations([
    /* confirmed array minus nt_mergetag */
  ]);
};
```
...
- [ ] **Step 3: Dry-run + apply.** `CONTENTFUL_ENVIRONMENT=master-1.0.0 node scripts/contentful/run.mjs 01 --dry-run` → review the plan → re-run without `--dry-run`.
...
- [ ] **Step 5: Verify rendered.** `CONTENTFUL_ENVIRONMENT=master-1.0.0 pnpm build` succeeds; home + community + blog render unchanged (nothing referenced `componentQuote`).
...
- [ ] **Step 1: Confirm the blog CTA machineName (MCP).** Against `master-1.0.0`: `get_entry` for the `page` whose `slug`/`machineName` is `blog`; read its `extraSection` reference → the linked `componentCta`'s `machineName`. Record it (call it `BLOG_CTA` below — siblings use `"connect-with-us"`; confirm whether blog reuses it or has its own).
...
- [ ] **Step 5: Verify rendered parity.** `CONTENTFUL_ENVIRONMENT=master-1.0.0 pnpm dev`; open `/es-AR/blog` and `/en-US/blog`; confirm the appended contact CTA renders identically to before (the only DOM delta is the removed wrapper `<div>`). Confirm home/community/come-meet-us untouched.
...
- [ ] **Step 7: Dry-run + apply + verify CMS.** Run migration `02` against `master-1.0.0`; MCP-confirm `componentDuplex` is 404 and `page` no longer has the three array fields but retains `internalName`/`pageName`/`slug`/`seo`. Confirm `componentCta.targetPage` / `componentHeroBanner.targetPage` / `menuGroup.groupLink` still resolve `page.slug`.
...
module.exports = function (migration) {
  ...
  belief
    ...
    .name("Bible Verse")
    .type("Link")
    .linkType("Entry"); // structured in Task 4
...
- [ ] **Step 6: Apply + verify CMS.** Run `03` against `master-1.0.0`. MCP: `beliefItem` count = 10; `credo`/`valueItem` = 404; the collections now link `beliefItem`s; `kind` set correctly (7 Creed, 3 Value).
...
- [ ] **Step 9: Verify rendered parity.** Local against `master-1.0.0`: `/es-AR` (home Values) and `/es-AR/community` (Creed) render identically — same items, order, text.
...
- [ ] **Step 3: Apply + verify CMS.** Run against `master-1.0.0`; MCP-confirm each `beliefItem` links a structured `bibleVerse` with correct fields.
...
- [ ] **Step 5: Verify rendered parity (the 🟡 to watch).** Local against `master-1.0.0`: community Creed + home Values verses look **identical** to before (same reference formatting, same text). Screenshot-compare if unsure.
...
- [ ] **Step 5: Apply + verify CMS.** Run against `master-1.0.0`; MCP-confirm 4 `section` entries (hero, 2× textBlock, cta) with correct `layout`, the textBlock sections retain `images`, and the three old types are 404.
...
- [ ] **Step 9: Verify rendered parity (CRITICAL — PhotoGrid).** Local against `master-1.0.0`: home hero, all three CTAs (home/community/blog-post), community text block **and its PhotoGrid** (guard at `community/page.tsx:66` must still fire), come-meet-us text block — all byte-for-byte identical. If the PhotoGrid vanished, `images` was dropped — fix the migration.
...
- [ ] **Step 3: Apply + verify CMS.** Run `06` against `master-1.0.0`; MCP-confirm both fields are gone.

- [ ] **Step 4: Drop the dead query field.** In `getContactForm.ts`, remove `validation` from `GRAPHQL_FIELDS` (line ~29).

- [ ] **Step 5: Verify rendered parity.** Local against `master-1.0.0`: navbar mega-menu groups and the contact form render unchanged.
...
- [ ] **Step 5: Verify rendered.** Local against `master-1.0.0`: `/es-AR/blog` shows Spanish category labels; `/en-US/blog` shows English. This is the approved 🟦 change.
...
- [ ] **Step 3: Apply (additive phase) + update getters.** Run the additive part; update `getNavigationMenu.ts`, the `menuGroup` fragment, and `getSeo.ts` to read the standardized fields. Verify locally against `master-1.0.0` that nav/footer/SEO metadata are unchanged.

- [ ] **Step 4: Apply (cleanup phase).** Delete the now-unused old fields (`menuGroup.internalTitle`, etc.).

- [ ] **Step 5: Verify rendered parity.** Local against `master-1.0.0`: navbar, footer, and `<head>` metadata identical.
...
- [ ] **Step 5: Verify rendered.** Local against `master-1.0.0`: heading shows Spanish on es-AR / English on en-US; the existing image+verse layout is unchanged; (manually) a verse with no image now appears.
...
- [ ] **Step 5: Verify rendered.** Local against `master-1.0.0`: `/es-AR/privacidad` and `/en-US/privacy` (or chosen slug) render the policy with correct metadata; footer link navigates there; existing routes unaffected. This is the approved 🟢 addition.
...
- [ ] **Step 3: Confirm the model target.** MCP against `master-1.0.0`: content-type count reduced to ~14 domain + composition + singleton types; `componentQuote`/`componentDuplex`/`credo`/`valueItem`/`componentHeroBanner`/`componentCta`/`componentTextBlock` are gone; `beliefItem`/`section` present; phantoms gone.

- [ ] **Step 4: Full gate.** `pnpm type-check && pnpm lint && pnpm test && pnpm build` green.

- [ ] **Step 5: Set the preview env var.** In Vercel, set `CONTENTFUL_ENVIRONMENT=master-1.0.0` scoped to the `refactor/ICR-76-contentful-model-optimization` branch's Preview, so the PR preview renders the new model.
...
- [ ] **Step 7: Hand off the cutover runbook.** In the PR description, paste the §3.3 runbook so the human re-points the `master` alias → `master-1.0.0` at merge time, with rollback (flip alias back) documented.
...
2. Pick a low-traffic minute → re-point `master` alias → `master-1.0.0` (Contentful → Settings → Environments → Aliases).
...
7. **Housekeeping (next cycle):** `master` now points at `master-1.0.0` (production). Delete stale `master-0.0.1`, clone `master-1.0.0` → the next versioned work env (per the bump class), and re-point the MCP `ENVIRONMENT_ID` + `.env.local` + branch Preview at it. See `docs/contentful-environments.md`.
