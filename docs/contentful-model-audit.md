...
- **T8 · Merge the promo blocks (`Hero` / `Cta` / `Duplex` / `TextBlock`) into one `section` type with a `layout` enum.** They share headline + rich text + ctaText + targetPage + image; they differ only in layout. Textbook "layout is a *field*, not a *type*." Only **5 entries** to remap. This is the biggest model simplification and the **only item with real code churn** (unifies the React components + getters), so it can land last / on its own. Pre-accepted low risk given the tiny dataset.
