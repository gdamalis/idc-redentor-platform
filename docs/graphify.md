...
1. **Git post-commit hook** (`.husky/post-commit`, tracked) — re-extracts changed **code** on every
   commit via AST. No LLM, no network, ~2s, detached so it never blocks/fails the commit. No-ops when
   graphify isn't installed, the checkout has no graph, or a `/work` update lock is held.
