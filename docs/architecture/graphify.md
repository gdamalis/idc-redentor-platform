...
1. **Git post-commit hook** (`.husky/post-commit`, tracked) — keeps the **shared graph in sync with
   ...
   commit. It resolves the **main repo root via the common git dir** (so it works from `/yoke:work`
   worktrees, where `--show-toplevel` would point at the worktree and skip the refresh) and refreshes
   the one shared graph there, lock-guarded against races and yielding to a `/yoke:work` update. No-ops when
   ...
   main** — which is also the tree `/yoke:work`'s agents query.
2. **`/yoke:work` per-session update** — the first `/yoke:work` that acquires the lock runs `graphify update`
