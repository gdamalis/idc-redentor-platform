...
- **`config/headers.js`** (via the pure `config/securityHeaders.js#buildSecurityHeaders`) applies to every path: `Strict-Transport-Security` (2-year, preload), `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `X-XSS-Protection`, `X-DNS-Prefetch-Control`, a **Content-Security-Policy**, and — **on production only** — `X-Frame-Options: SAMEORIGIN`.
- **The CSP `frame-ancestors` is environment-gated** (from `VERCEL_ENV`/`NODE_ENV`) so Contentful Live Preview can iframe the site **without** weakening production's clickjacking protection: **production** is strict — `frame-ancestors 'self'` + `X-Frame-Options: SAMEORIGIN` (Contentful **cannot** frame prod); **preview/dev** drops `X-Frame-Options` (it silently blocks the iframe even with a correct CSP) and allows `frame-ancestors 'self' https://app.contentful.com https://app.eu.contentful.com` so editors preview in-context. See [`contentful-data-layer.md`](./contentful-data-layer.md) § Live Preview. `script-src`/`connect-src` allowlist Google Tag Manager / Analytics and Vercel scripts; `img-src` allowlists the Contentful image CDNs (`images.ctfassets.net`, `images.eu.ctfassets.net`) and `images.unsplash.com`. `next.config.ts` mirrors those image hosts in `images.remotePatterns`.
